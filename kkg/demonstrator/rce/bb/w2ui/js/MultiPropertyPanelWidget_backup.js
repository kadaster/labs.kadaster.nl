	         ...
	         else
	             {
                    if (data.startsWith("//file/"))
                    {
                        ...
                        dInfo=dInfo[dInfo.length-1];
                          html=html.replace(">"+data+"</div>","><a href=\"javascript:MyCommand.downloadFile('"+data+"');\">"+dInfo+info+"</a></div>"); 
                    }
                    else
                    {
...
var MultiPropertyPanelWidget=function(layout,panel,widget,rv)
{
   ...
   // console.log(this);
    ...
    this.grid=$().w2grid({ 
        ...
        columns: [                
            { field: 'prop', sortable: true,text: 'Property', size: '50%' ,render:multipopertypanelrender },
            { field: 'value', text: 'Value', size: '50%',   editable: { type: 'text' } , render:multipopertypanelrenderValueCol},
        ]
    });  
	...
	if (item.w2ui.loaded==false)
     ...
     {
         //console.log("replacing based upon childParameter index 2")
         ...
         sparql = sparql.replace(me.childParameterText,arg);
	      ...
	      // console.log("expanding",me,sparql);
