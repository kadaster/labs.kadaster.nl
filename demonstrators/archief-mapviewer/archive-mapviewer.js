...
function getParcelMetaData(municipalityCode, section, parcelNumber) {
  const url =
    "https://api.labs.kadaster.nl/datasets/kadaster/brk/services/brk/sparql";
  const query = `
...
fetch("parcels-to-fieldwork.json")
  .then(res => res.json())
  .then(lookupTable => {
    parcelsToFieldwork = lookupTable;
  });
...
${parcelMetaData.municipalityCode} ${parcelMetaData.section} ${parcelMetaData.parcelNumber}
  ...
  const foundFieldwork = parcelsToFieldwork
    ...
    .filter(
      entry =>
        fieldworkFiles.filter(f => f.veldwerk === entry.Veldwerk).length > 0
    );

  const foundFiles = foundFieldwork.map(entry =>
    fieldworkFiles.filter(f => f.veldwerk === entry.Veldwerk)
  );

  if (foundFieldwork.length > 0) {
    foundFieldwork.forEach((found, index) => {
      innerHTML += `Veldwerk ${found.Veldwerk.toString()}<br>`;
      foundFiles[index].forEach(file => {
...
document
  ...
  .addEventListener("click", setMapToFullScreen);
