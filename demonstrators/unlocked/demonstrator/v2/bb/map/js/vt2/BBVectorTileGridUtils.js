...
var vtBuffer=264;
var vtDebug=1; // off
var vtTolerance=0; // default 4 // necessary for points as circles // 1 for full view with esri 
...
return [100,100,100];
 ...
 Singleton.getInstance().selectionColor="RGB(255,0,0)";
...
function getKleur(feature)
{
    ...
    try
    {
        ...
        return Singleton.getInstance().selectionColor;
        }
    }
    catch(error) {console.log(error);}
 ...
 //  console.log(feature,feature.tags.uri,Singleton.getInstance().selectedUri);
	...
	if (feature.tags.color!=null)
	    {
	       if ( (feature.tags.color.startsWith("RGBA")) || (feature.tags.color.startsWith("rgb")) ) {
            kleur=feature.tags.color;
...
function drawTileOnCanvas(tile,ctx,pad,color)
{
    ...
    var thickness=document.mapsize*2 ;
    if (document.sparql!=null)
    {
        ...
        try
        {
            if (document.sparql.includes("#thickness:"))
            {
                 thickness =parseInt(document.sparql.split("#thickness:")[1].split(";")[0]);
    ...
    for (let iii in features) {
        ...
        for (let j in feature.geometry) {
            ...
            else
        	{
        	  ...
        	  if (type === 2) { ctx.lineWidth = thickness; ctx.strokeStyle=getKleur(feature);	}
              else
              {
                ctx.lineWidth = 1;
                ctx.globalAlpha = 0.1;
            ...
            for (let k in geom) {
              ...
              //  if (k) ctx.lineTo(x  + pad, y   + pad);
               // else ctx.moveTo(x  + pad, y  + pad);
                ...
                else {ctx.moveTo(x-1  , y  ); begin=false;}
