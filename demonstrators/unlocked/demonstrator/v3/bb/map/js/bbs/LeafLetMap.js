...
function resparql(layer) {
  ...
  try {
    Singleton.getInstance().scope.loader.map = true
  } catch (error) {
  }
  ...
  if (Singleton.getInstance().sparql != null) {
    if (layer == null) {
      ...
      try {
        ...
        var points = [bounds.getSouthWest(), bounds.getNorthWest(), bounds.getNorthEast(), bounds.getSouthEast()]
      }
...
function featureSelected(e, o) {
  ...
  try {
    ...
    var size = 5
    try {
      size = o.options.icon.options.iconSize[0]
    } catch (error) {
    }
  ...
  try {
    ...
    if (Singleton.getInstance().map.redDots != null) {
      for (var n in Singleton.getInstance().map.redDots) {
        ...
        var size = 5
        try {
          size = redDot.options.icon.options.iconSize[0]
        } catch (error) {
        }
