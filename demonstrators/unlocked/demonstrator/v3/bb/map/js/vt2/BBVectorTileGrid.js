...
L.GridLayer.GeoJSON = L.GridLayer.extend({
  ...
  smartRedraw: function() {
    ...
    const col = getHighestZIndexChildren(this._container)
    var pad = 0
    ...
    */
    ...
    if (this.options.mouseEvents != false) {
      tile.addEventListener('click', function(e) {
        handleMouseClick(e, this, ctx, tile, pad)
      }, false)
    }
      ...
      if (isRealistic) {
        if (this.options.mouseEvents != false) {
          tile.addEventListener('mousemove', function(e) {
            mouseMoveEventCanvasCursorSelector(e, this, ctx, tile, pad)
          }, false)
        }
      }
    }
...
function getHighestZIndexChildren(container) {
  const tileContainers = Array.from(container.children)
  let highestZIndex = -Infinity
  let highestZIndexContainer = null

  tileContainers.forEach(child => {
    const zIndex = parseInt(child.style.zIndex, 10) || 0
    if (zIndex > highestZIndex) {
      highestZIndex = zIndex
      highestZIndexContainer = child
    }
  })

  return highestZIndexContainer.children
}
