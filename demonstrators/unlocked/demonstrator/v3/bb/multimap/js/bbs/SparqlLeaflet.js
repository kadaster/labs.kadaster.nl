	...
	//  ar.rgbcolor=true;
	 ...
	 ar.colorOVolgorde=[];
	  ...
	  // use rgb mechanism for this.
		...
		try 
	      	{
		         console.log("beschrijving part 2",keys);
...
function processJson(json,sparql)
{
 ...
 //console.log("sparql geanalyseerd");
   ...
   try {   Singleton.getInstance().legend.update(ar); } catch(e){console.log("legend update error",e);}
...
function getBinding(uri)
{
 ...
 function processPMTiles()
 {
  console.log("pm tiles Not working really well");
  if (true) return;
   
   addPMTile("http://localhost:3000/temp/bomen.pmtiles");
    addPMTile("http://localhost:3000/temp/wegen1.pmtiles");
    addPMTile("http://localhost:3000/temp/wegen2.pmtiles");
   addPMTile("http://localhost:3000/temp/algemeen.pmtiles");
   addPMTile("http://localhost:3000/temp/groen1.pmtiles");
   addPMTile("http://localhost:3000/temp/groen2.pmtiles");
   addPMTile("http://localhost:3000/temp/water.pmtiles");
   addPMTile("http://localhost:3000/temp/rio1.pmtiles");
   addPMTile("http://localhost:3000/temp/rio2.pmtiles");
   addPMTile("http://localhost:3000/temp/rio3.pmtiles");
   addPMTile("http://localhost:3000/temp/borden.pmtiles");

 }
 function addPMTile(url)
 {
  var layer = protomapsL.leafletLayer({url:url})
  layer.addTo(map);
}
 ...
 function parsejson(json,sparql)
{
  ...
  //  console.log("parsejson");
    ...
    if (sparql.includes("#geojson;"))
    {
        processDeckGLGeoJSON(json,sparql);
    ...
    else
    {
        ...
        {
          processDeckGLHeatmap(json,sparql);
          return;
  ...
  // console.log("parsing json 22");
     ...
     // console.log("parsing json 3");
  	try {if (Singleton.getInstance().map.getZoom()>20) {setTimeout(function(){Singleton.getInstance().map.setZoom(20);},100);} } catch(eee) {console.log(eee);}
