	...
	var methods = {
		...
		parseRawResourceDoc : function(destBox, URI) {
			...
			if (lodLiveProfile['default']) {
				...
				var url = lodLiveProfile['default'].endpoint + "?" + (value.endpointType ? $.jStorage.get('endpoints')[value.endpointType] : $.jStorage.get('endpoints')['all'])   +   "&uri=" + encodeURIComponent(URI) + "&query=" + encodeURIComponent(res);
				 //url = value.endpoint + "?" + (value.endpointType ? $.jStorage.get('endpoints')[value.endpointType] : $.jStorage.get('endpoints')['all']) + "&query=" + encodeURIComponent(res);
		...
		drawaLine : function(from, to, propertyName) {
			...
			console.debug(new Date().getTime()+'moving - '+(new Date())+" - #line-" + from.attr("id") + "-" + to.attr("id"))
			if (aCanvas.length == 1) {
				if (propertyName) {
